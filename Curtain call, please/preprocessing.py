# ===============================================================
# Reel Patterns: Curtain Call, Please - Data preprocessing module
# ===============================================================
...
def calculate_index_in_group(wikidata_movies_df: pd.DataFrame) -> pd.DataFrame:
    ...
    :param wikidata_movies_df: A pandas DataFrame containing movie data.
                               It must include the columns `RELEASE_DATE` and `GROUP_ID`.
    :return: The input DataFrame with an additional column `INDEX_IN_GROUP`,
             which contains the calculated index for each row within its group.
    :raises KeyError: If the required columns `RELEASE_DATE` or `GROUP_ID` are missing
    """
...
def preprocess_data(rt_movies_df: pd.DataFrame,
                    wikidata_movies_df: pd.DataFrame,
                    imdb_movies_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    :param rt_movies_df: DataFrame containing Rotten Tomatoes movie data.
    :param wikidata_movies_df: DataFrame containing Wikidata movie data.
    :param imdb_movies_df: DataFrame containing IMDB movie data.
    :return: A preprocessed DataFrame with integrated and enriched movie data.
    """
    ...
    wikidata_movies_df = handle_missing_revenue_and_budget(wikidata_movies_df, imdb_movies_df)
    ...
    # Calculate the index of movie within its group based on release date
    wikidata_movies_df = calculate_index_in_group(wikidata_movies_df)

    return wikidata_movies_df


def handle_release_dates(wikidata_movies_df: pd.DataFrame, imdb_movies_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    from the `IMDB_RELEASE_DATE` column of `imdb_movies_df`.
    Finally, it ensures that the `RELEASE_DATE` column is unified and contains the most complete data available.
    :param wikidata_movies_df: A DataFrame containing movie data from Wikidata, including a `RELEASE_DATE` column.
    :param imdb_movies_df: A DataFrame containing movie data from IMDB, including `IMDB_ID` and `IMDB_RELEASE_DATE` columns.
    :return: The updated `wikidata_movies_df` DataFrame with missing release dates filled where possible.
    """
...
def handle_groups_sizes(wikidata_movies_df: pd.DataFrame) -> pd.DataFrame:
    ...
    :param wikidata_movies_df: A DataFrame containing movie data. Must include `GROUP_ID` column.
    :return: A filtered DataFrame containing only movies that are part of groups with more than one movie.
             The DataFrame includes an additional column (N_FILMS_IN_GROUP) indicating the size of each group.
    """
    ...
    valid_number_of_films_in_group: pd.Series[bool] = wikidata_movies_df[N_FILMS_IN_GROUP] > 1
    wikidata_movies_df = wikidata_movies_df[valid_number_of_films_in_group]

    return wikidata_movies_df


def handle_missing_revenue_and_budget(wikidata_movies_df: pd.DataFrame, imdb_movies_df: pd.DataFrame) -> pd.DataFrame:
    """
    Handles missing revenue and budget data in a DataFrame by merging data from another DataFrame
    and unifying the budget and revenue columns, while also calculating the ROI (Return on Investment).
    This function performs the following steps:

    1. Merges `wikidata_movies_df` with `imdb_movies_df` on `IMDB_ID` to fill missing budget and revenue values.
    ...
    :param wikidata_movies_df: A DataFrame containing movie data from Wikidata,
                               including columns for budget and revenue.
    :param imdb_movies_df: A DataFrame containing movie data from IMDB,
                           including columns for budget, revenue, and IMDB IDs.
    :return: A DataFrame with unified budget and revenue columns, an additional ROI column,
    ...
    # Remove rows with missing or zero budget or box office value
    rows_to_remove: pd.Series[bool] = (
...
def unify_series_and_franchise_columns(wikidata_movies_df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    :param wikidata_movies_df: A DataFrame containing movie data, including series and franchise information.
    :return: A cleaned and processed DataFrame with unified series and franchise columns.
    """
    ...
    wikidata_movies_df = remove_series_and_franchises(wikidata_movies_df, series_to_remove, franchises_to_remove)
...
def remove_invalid_movies(wikidata_movies_df: pd.DataFrame) -> pd.DataFrame:
    ...
    :return: A filtered DataFrame containing only valid movies.
    """
    part_of_series_or_franchise: pd.Series[bool] = (
            (wikidata_movies_df[SERIES_ID].notna()) | (wikidata_movies_df[FRANCHISE_ID].notna())
    )

    have_valid_wikidata_qid: pd.Series[bool] = wikidata_movies_df[WIKIDATA_FILM_ID].notna()

    wikidata_movies_df = wikidata_movies_df[part_of_series_or_franchise & have_valid_wikidata_qid]
    return wikidata_movies_df
...
def remove_bad_series_labels(wikidata_movies_df: pd.DataFrame) -> pd.DataFrame:
    ...
    by the `constants.BAD_SERIES_LABELS` list).
    For these rows, it sets the `SERIES_LABEL` and `SERIES_ID` columns to `pd.NA`.
    :param wikidata_movies_df: A pandas DataFrame containing movie data,
                               including `SERIES_LABEL` and `SERIES_ID` columns.
    :return: The modified DataFrame with invalid series labels and IDs removed.
    """
    # Remove rows with bad series labels
    bad_series_rows: pd.Series[bool] = wikidata_movies_df[SERIES_LABEL].isin(BAD_SERIES_LABELS)
...
def find_all_franchises_and_series_in_connected_component(wikidata_movies_df: pd.DataFrame,
                                                          franchise_label: Optional[str] = None,
                                                          series_label: Optional[str] = None) -> Tuple[Set[str], Set[str]]:
    """
    Identifies all franchises and series connected to a given franchise or series within a dataset of movies.
    The function performs a breadth-first search to traverse the connected components of franchises and series.
    :param wikidata_movies_df: A DataFrame containing movie data with columns for franchise and series labels.
    :param franchise_label: The starting franchise label to search for connected components.
    :param series_label: The starting series label to search for connected components.
    :return: A tuple containing two sets:
             - franchises_visited: A set of all franchise labels connected to the starting franchise or series.
             - series_visited: A set of all series labels connected to the starting franchise or series.
    """
    franchises_visited: Set[str] = set()
    franchises_to_check: List[str] = [] if pd.isna(franchise_label) else [franchise_label]
    series_visited: Set[str] = set()
    series_to_check: List[str] = [] if pd.isna(series_label) else [series_label]

    # Breadth-first search to find all connected franchises and series
    while franchises_to_check or series_to_check:
        rows: pd.Series = pd.Series(False, index=wikidata_movies_df.index)

        if franchises_to_check:
            curr_franchise: str = franchises_to_check.pop()
            rows |= wikidata_movies_df[FRANCHISE_LABEL] == curr_franchise
            franchises_visited.add(curr_franchise)

        if series_to_check:
            curr_series: str = series_to_check.pop()
            rows |= wikidata_movies_df[SERIES_LABEL] == curr_series
            series_visited.add(curr_series)

        related_movies: pd.DataFrame = wikidata_movies_df[rows]  # Movies related to the current franchise or series

        # Find new franchises connected to the current ones
        ...
        # Find new series connected to the current ones
...
def decide_who_to_remove_based_on_containment(df: pd.DataFrame, series_label, franchise_label) -> int:
    """
    Decides whether to remove a series or a franchise based on their containment relationship.
    The function compares the sets of movies associated with the given series and franchise labels.
    It determines which entity to remove based on the following rules:
    - If the sets are equal, remove the franchise.
    - If the franchise's set is a subset of the series' set, remove the franchise.
    - If the series' set is a subset of the franchise's set, remove the series.
    - If neither set is a subset of the other, remove the entity with the larger set
    :param df: A DataFrame containing movie data with columns for series and franchise labels.
    :param series_label: The series label to evaluate.
    :param franchise_label: The franchise label to evaluate.
    :return: An integer indicating which entity to remove:
             - REMOVE_FRANCHISE (1) if the franchise should be removed.
             - REMOVE_SERIES (2) if the series should be removed.
    """
    series_items: Set[str] = set(df.index[df[SERIES_LABEL] == series_label])
    franchise_items: Set[str] = set(df.index[df[FRANCHISE_LABEL] == franchise_label])
    ...
    return REMOVE_FRANCHISE if len(series_items) >= len(franchise_items) else REMOVE_SERIES


def calculate_franchises_and_series_to_remove(wikidata_movies_df: pd.DataFrame) -> Tuple[Set[str], Set[str]]:
    """
    ...
    :param wikidata_movies_df: A DataFrame containing movie data with columns for series and franchise labels.
    :return: A tuple containing two sets:
             - series_to_remove: A set of series labels to be removed.
             - franchises_to_remove: A set of franchise labels to be removed.
    """
    already_found_franchises: Set[str] = set()
    already_found_series: Set[str] = set()

    series_to_remove: Set[str] = set()
    franchises_to_remove: Set[str] = set()

    series_franchise_pairs: pd.DataFrame = wikidata_movies_df[[SERIES_LABEL, FRANCHISE_LABEL]].dropna().drop_duplicates()

    for series, franchise in series_franchise_pairs.itertuples(index=False, name=None):
        ...
        if connected_franchises.issubset(already_found_franchises) and connected_series.issubset(already_found_series):
            # Already processed this connected component
            continue

        sum_of_lengths = len(connected_franchises) + len(connected_series)

        containment_relation_result: int = -1  # Unknown yet
...
def remove_series_and_franchises(wikidata_movies_df: pd.DataFrame, series_to_remove: Set[str],
                                 franchises_to_remove: Set[str]) -> pd.DataFrame:
    """
    ...
    :param wikidata_movies_df: A DataFrame containing movie data with columns for series and franchise labels and IDs.
    :param series_to_remove: A set of series labels to be removed from the DataFrame.
    :param franchises_to_remove: A set of franchise labels to be removed from the DataFrame.
    :return: A filtered DataFrame where the specified series and franchises have been removed,
             and movies without any series or franchise are excluded.
    :raises AssertionError: if the resulting DataFrame contains rows where both series
             and franchise information are missing or both are present.
    """
    for series_label in series_to_remove:
        rows: pd.Series[bool] = wikidata_movies_df[SERIES_LABEL] == series_label
    ...
    for franchise_label in franchises_to_remove:
        rows: pd.Series[bool] = wikidata_movies_df[FRANCHISE_LABEL] == franchise_label
    ...
    # Assert that each movie is now either part of a series or a franchise, but not both
...
def create_single_grouping_column(wikidata_movies_df: pd.DataFrame) -> pd.DataFrame:
    ...
    :param wikidata_movies_df: A pandas DataFrame containing movie data with columns
                               FRANCHISE_LABEL, SERIES_LABEL, FRANCHISE_ID, and SERIES_ID.
    :return: The modified DataFrame with the new GROUP_LABEL and GROUP_ID columns added.
    """
