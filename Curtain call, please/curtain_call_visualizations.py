...
def prepare_dataframe_for_probability_of_success_calculation(
    data: pd.DataFrame,
    metric: Metric = ROI,
    max_length_of_group: int = 6,
    min_length_of_group: int = 0,
):
    """
    ...
    Args:
        ...
        metric (Metric, optional): The column representing the metric to be analyzed.
            Defaults to ROI.
        max_length_of_group (int, optional): The maximum number of movies allowed in a group.
            Defaults to 6.
        min_length_of_group (int, optional): The minimum number of movies required in a group.
            Defaults to 0.
    Returns:
        pd.DataFrame: A cleaned and filtered DataFrame ready for probability of success calculation.
    Raises:
        AssertionError: If the `metric` column contains missing or infinite values.
    Notes:
        ...
        - Rows with missing values in the `GROUP_ID` column are dropped, and the DataFrame
          is sorted by `GROUP_ID` and `RELEASE_DATE`.
    """

    df = data.copy()
    # Keep only groups with at least min_length_of_group and at most max_length_of_group movies
    df = df[
        (df[N_FILMS_IN_GROUP] >= min_length_of_group)
        & (df[N_FILMS_IN_GROUP] <= max_length_of_group)
    ]
    # Assert no missing or inf values in metric column
    assert (
        df[metric].notna().all() & (np.isfinite(df[metric])).all()
...
def calculate_dataframe_for_probability_of_success(
    df: pd.DataFrame,
    metric: Metric = ROI,
    ...
    Args:
        df (pd.DataFrame): Input DataFrame containing movie data. Must include columns for
            group IDs, metric values, and index within the group.
        metric (Metric, optional): The performance metric to evaluate (e.g., ROI). Defaults to ROI.
        tolerance_for_successful (int, optional): The percentage gap tolerance for a movie to be
            considered as successful as the best-so-far. Defaults to 0.
        m (int, optional): The prior weight for the Bayesian estimate. Defaults to 5.
        alpha (float, optional): The significance level for the Wilson confidence interval.
            Defaults to 0.2.
    Returns:
    ...
    Notes:
        ...
        - Rows without a previous best metric value (e.g., the first movie in each franchise)
    ...
    df[BEST_METRIC_VALUE_SO_FAR] = df.groupby(GROUP_ID, sort=False)[
        BEST_METRIC_VALUE_SO_FAR
    ].shift(1)

    # Calculate % gap vs best-so-far (positive means >= best-so-far)
    df[PERCENT_OF_BEST_SO_FAR] = 100.0 * (
        df[metric] / df[BEST_METRIC_VALUE_SO_FAR] - 1.0
    )
    ...
    df.loc[:, AS_SUCCESSFUL_AS_BEST_SO_FAR] = (
        df[PERCENT_OF_BEST_SO_FAR] >= tolerance_for_successful
    ).astype(int)
...
def plot_probability_of_success(
    data: pd.DataFrame,
    metric: Metric = ROI,
    max_length_of_group: int = 6,
    min_length_of_group: int = 0,
    ...
    the dataset, along with confidence intervals. The x-axis represents the index in the
    franchise, and the y-axis represents the probability of success.
    Args:
        data (pd.DataFrame): The input data containing the relevant metrics and group information.
        metric (Metric, optional): The metric to evaluate the probability of success. Defaults to ROI.
        max_length_of_group (int, optional): The maximum length of groups to consider. Defaults to 6.
        min_length_of_group (int, optional): The minimum length of groups to consider. Defaults to 0.
        tolerance_for_successful (int, optional): The tolerance level for defining success. Defaults to 0.
        m (int, optional): The smoothing parameter for Bayesian shrinkage. Defaults to 5.
        alpha (float, optional): The significance level for confidence intervals. Defaults to 0.2.
        title (str, optional): The title of the plot. Defaults to the title mapped to the ROI metric.
    Returns:
    ...
    Example:
        >>> plot_probability_of_success(data, metric=ROI, max_length_of_group=6, min_length_of_group=1)
    """

    df = prepare_dataframe_for_probability_of_success_calculation(
        data, metric, max_length_of_group, min_length_of_group
    )
    success_prob_summary = calculate_dataframe_for_probability_of_success(
        df, metric, tolerance_for_successful, m, alpha
    )

    X = success_prob_summary[INDEX_IN_GROUP]
    Y_SHRUNK = success_prob_summary[BAYESIAN_SHRUNKEN_PROB_OF_SUCCESS]
    CI_LO = success_prob_summary[CI_LOWER]
    CI_HI = success_prob_summary[CI_UPPER]
    ...
    ax.plot(
        X + 1, Y_SHRUNK, marker="o", color=MAIN_COLOR, linewidth=2, label="Probability"
    )
...
def plot_movies_per_sequel_index(data: pd.DataFrame):
    """
    ...
    visualizes the distribution as a bar chart. The chart includes labeled axes, a title,
    and a grid for better readability. The plot is saved to a specified file path and displayed.

    Args:
        data (pd.DataFrame): A pandas DataFrame containing the data to be plotted.
                             It must include a column corresponding to sequel indices.

    Raises:
        KeyError: If the required column `INDEX_IN_GROUP` is not present in the DataFrame.
        FileNotFoundError: If the directory for saving the plot does not exist.

    Notes:
        - The bar chart uses a red color scheme with black edges for the bars.
        - The x-axis represents the sequel index, and the y-axis represents the number of movies.
        - The plot is saved to the path specified by the constant `MOVIES_PER_SEQUAL_PLOT_OUT_PATH`.
        - The figure layout is adjusted to prevent overlapping elements.

    Constants:
        INDEX_IN_GROUP (str): The column name in the DataFrame representing the sequel index.
        MOVIES_PER_SEQUAL_PLOT_OUT_PATH (str): The file path where the plot will be saved.
...
if __name__ == "__main__":
    # Load all dataframes
    rt_movies_df = pd.read_csv(ROTTEN_TOMATOES_PATH)
    wikidata_movies_df = pd.read_csv(WIKIDATA_PATH)
    imdb_movies_df = pd.read_csv(IMDB_PATH)

    data = preprocess_data(rt_movies_df, wikidata_movies_df, imdb_movies_df)

    plot_movies_per_sequel_index(data)
    plot_probability_of_success(data, metric=ROI, title=TITLE_MAP[ROI])
    plot_probability_of_success(
        data, metric=AUDIENCE_RATING, title=TITLE_MAP[AUDIENCE_RATING]
    )
    plot_probability_of_success(
        data, metric=CRITIC_RATING, title=TITLE_MAP[CRITIC_RATING]
    )
